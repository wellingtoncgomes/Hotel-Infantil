module.exports = {
  ...
  create: (pool, data, callback) => {
    ...
    const sql =
      "INSERT INTO cardapio (nome, descricao, tipo, dia_semana, imagem) VALUES ($1, $2, $3, $4, $5)";
  ...
  update: (pool, data, callback) => {
    ...
    console.log(data);
    const sql =
      "UPDATE Cardapio SET nome = $1, descricao = $2, tipo = $3, dia_semana = $4, imagem = $5 WHERE id_item = $6";
    pool.query(
      sql,
      [nome, descricao, tipo, dia_semana, imagem_url, id],
      (err) => {
        if (err) return callback(err);
        callback(null);
      }
