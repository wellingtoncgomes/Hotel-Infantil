...
module.exports = {
    ...
    createReserva: async (req, res) => {
        ...
        try {
            ...
            if (!error) {
                const relacaoValida = await reservasModel.verificarRelacaoPaiFilho(pool, id_crianca, id_pai);
                if (!relacaoValida) {
            ...
            if (!error) {
                await new Promise((resolve, reject) => {
                    reservasModel.verificarPagamentoPendente(pool, id_pai, (err, temPagamentoPendente) => {
                        if (err) return reject(err);
                        if (temPagamentoPendente) {
                            error = "O pai possui pagamentos pendentes. Não é possível realizar a reserva.";
                        }
                        resolve();
                    });
                });
            ...
            if (!error) {
                await reservasModel.create(pool, { id_crianca, id_pai, id_atividade, data_reserva });
    ...
    removeReserva: (req, res) => {
        ...
        reservasModel.checkPagamento(pool, id, (err, hasPagamento) => {
            ...
            if (hasPagamento) {
                ...
                return res.status(400).send('Não é possível excluir a reserva, pois há pagamento atrelado.');
            }
